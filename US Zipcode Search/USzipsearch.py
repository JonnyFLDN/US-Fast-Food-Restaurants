...
class Zipcode_Search(object):
    ...
    def __init__(self,shapefile_location=None):
        ...
        if shapefile_location == None:
            ...
            self.shapefile_reader = shapefile.reader(shapefile_location)
    ...
    def download_files(self):
        ...
        with zipfile.ZipFile(io.BytesIO(r.content)) as z:
            ...
            self.shapefile_reader = shapefile.Reader(shp=io.BytesIO(z.read(ext_d['.shp'])),
    ...
    def get_shapes(self,zip_find=None):
        ...
        field_num = self.shapefile_reader.fields[1:][0].index('ZCTA5CE10')
    ...
    def coord_neighbour(self,lat_lon,n_num =5,zip_restrict =None):
        ...
        #Load r-tree 
        index_tree = index.Index(list((i,tuple(obj.bbox),None) for i,obj, 
                                  in enumerate(shape_dic.itervalues())))
        
        #Grab a minimum of 5 nearest neighbours
        srch_num = max(5,n_num)
        all_zip = np.asarray(shape_dic.keys())
